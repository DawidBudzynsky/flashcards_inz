...
import { User, FlashcardSet, Folder, Flashcard, FlashcardToRevise } from "../types/interfaces";
                            ...
                            <li key={flashcard.FlashcardID}>
                                < div className="modal-box max-w-7xl w-full rounded-3xl space-y-5" >
                                    <div className='flex justify-end'>
                                        <span>{calculateTimeDifference(flashcard.NextReviewDue)}</span>
                                    </div>
                                    <div className='font-semibold'>
                                        {/* Get the count of flashcards for this FlashcardSetID */}
                                        {flashcardsToday.filter(f => f.Flashcard.FlashcardSetID === flashcard.Flashcard.FlashcardSetID).length}
                                        card to revise in set

                                        {/* Assuming FlashcardSets array or API provides the name */}
                                        <a
                                            href={`/flashcards_sets/${flashcard.Flashcard.FlashcardSetID}/learn`}
                                            className="text-blue-500 hover:underline"
                                        >
                                            {`Set ${flashcard.Flashcard.FlashcardSetID}`}  {/* You can replace with actual set name */}
                                        </a>
                                    </div>
                                </div>
