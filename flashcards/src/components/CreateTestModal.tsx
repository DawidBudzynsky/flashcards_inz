...
const CreateTestModal: React.FC = () => {
    ...
    // Filter out the sets that are already selected
    const availableSets = flashcardSets.filter(
        (set: FlashcardSet) => !selectedSets.some((selectedSet) => selectedSet.ID === set.ID)
    );


    const handleAddSet = (set: FlashcardSet) => {
        if (selectedSets.length < 5 && !selectedSets.find((s) => s.ID === set.ID)) {
    ...
    const { mutate } = useMutation({
        mutationFn: (data: { setIDs: number[]; dueDate: string }) => createTest(data),
        onSuccess: () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const requestData = {
            setIDs: selectedSets.map((set) => set.ID),  // Extract IDs from selectedSets
            dueDate: dueDate,
        };
    ...
    const totalFlashcards = selectedSets.reduce((total, set: FlashcardSet) => total + (set.Flashcards.length || 0), 0);
    ...
    return (
        <>
            ...
            <dialog id="create_test_modal" className="modal">
                <form onSubmit={handleSubmit} className="modal-box w-11/12 max-w-5xl h-1/2 max-h-xl">
                    <div className="grid grid-cols-2 gap-8">

                        {/* Left side */}
                        <div>
                            <h3 className="font-bold text-lg mb-4">Create New Test</h3>

                            <div className="gap-5 p-4 bg-gray-100 rounded-lg border border-gray-300 shadow-md">
                                <p className='font-bold text-sm pb-2'>Sets used for test:</p>
                                {/* Flashcards count */}
                                ...
                                <ul className="space-y-2">
                                    {selectedSets.length > 0 ? (
                                        selectedSets.map((set: FlashcardSet) => (
                                            <li
                                                key={set.ID}
                                        ...
                                        <p className="text-sm text-gray-500">No sets selected yet.</p>
                                    )}
                                </ul>
                            </div>
                        ...
                        <div>
                            <div className="mb-4">
                                <h4 className="font-semibold mb-2">Set Due Date</h4>
                                <input
                                    type="date"
                            ...
                            <div className="dropdown dropdown-bottom w-full relative">
                                <div tabIndex={0} className="btn btn-outline w-full">
                                    Pick Sets
                                </div>
                                <ul
                                    tabIndex={0}
                                ...
                                >
                                    {availableSets.length > 0 ? (
                                        availableSets.map((set: FlashcardSet) => (
                                            <li key={set.ID}>
                                                <a
                                        ...
                                        <p className="text-sm text-gray-500">No available sets left.</p>
                                    )}
                                </ul>
                            </div>
