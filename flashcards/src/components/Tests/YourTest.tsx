...
const YourTest: React.FC<YourTestProp> = ({ test }) => {
  ...
  useEffect(() => {
    const updateStatus = () => {
      const { status, countdown } = getTestStatus(test.StartDate, test.DueDate);
        ...
        <div className="flex flex-col md:h-72 bg-base border-[1px] transition-all duration-300 ease-in-out">
          ...
          <div className={"md:flex md:flex-grow gap-4 p-4"}>
            ...
            <div className="md:flex-1 text-center">
              {test.AssignedUsers.map((user: User) => (
                <div className="text-underline" key={user.Email}>
                  <button
                    className="underline text-blue-500"
                    onClick={() => navigate(`/users/${user.google_id}`)}
                  >
                    {user.Username}
                  </button>
                </div>
          ...
          <button
            className={`bg-base-content text-white py-4 md:flex items-center justify-center ${
              testStatus === "Open" ? "cursor-pointer" : "cursor-not-allowed"
            }`}
            onClick={handleTakeTest}
            disabled={testStatus !== "Open"}
          >
            <span className="font-semibold">
              {testStatus === "Open" ? "Take a test!" : countdown || testStatus}
            </span>
          </button>
        </div>
      </div>
